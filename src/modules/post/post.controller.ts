...
import paginationsorting from "../../helpers/paginationBySorting";
...
const getAllPostBySearch=async(req:Request,res:Response)=>{
    ...
    try {
        ...
        const result = await postService.findAllPostBySearch(searchString as string,tags as string[],isFeatured ,status as PostStatus |undefined,page,limit,skip,sortBy,sortOrder)
        res.status(200).json({
                success:true,
                message:'get all post successfully',
                data:result
